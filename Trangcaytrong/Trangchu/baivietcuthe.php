	...
	<!-- header-->
	 ...
	 <div class="row" style="margin-top: 50px;margin-bottom: 20px;">
	    	...
	    	<center>
		    	<div class="containers" >
			  	  <div>
			  	  	...
			  	  	<a  href="./trangcaytrong.html" style="color: black;text-decoration: none"><span> Danh mục sản phẩm</span></a>	
			  	  </div>
	    ...
	    <div class="col-sm-9"  style="background-color:#d5ecab;">
	    	...
	    	if(isset($_GET['id'])){
	    	$c1=$_GET['id'];
	    	$a112 = "SELECT * FROM tb_baiviet where Id='$c1'";
            $result112 = $conn->prepare($a112);
            $result112->execute();
            foreach ($result112 as $row){
	    	echo'<div class="sanpham" style="text-align:center;">
	    	<h1>'.$row['Title'].'</h1>
	    	<img src="'.$row['AnhMoTa'].'">
	    	<p>'.$row['MoTa'].'</p>
	    	<img src="'.$row['AnhNoiDung'].'">
	    	<p>'.$row['NoiDung'].'</p>
			<img src="'.$row['AnhKetThuc'].'">
			<p>'.$row['KetThuc'].'</p>
			</div>';}}
			?>
	    </div>
